...
simul_df <- 
  ...
  mutate(simul_dataset = map(sample_size, \(sa) rnorm(n = sa, mean = 0, sd = 5))) |> 
  mutate(summary = map(simul_dataset, \(si) t.test(x = si, mu = 0, conf.level = 0.95))) |> 
  unnest(summary)
...
simul_df <- 
  ...
  mutate(summary = map(simul_dataset, \(si) t_test(x = si))) |> 
  unnest(summary) |> 
  select(sample_size, true_mean_u, iter_simul_dataset, estimate, p.value)
...
head(simul_df)
  ...
  mutate(summary = map(simul_dataset, \(si) t.test(x = si, mu = 0, conf.level = 0.95))) |> 
  mutate(summary = map(summary, broom::tidy()))
  unnest(summary)

t_test(a)
         
head(simul_df)
a = rnorm(n = 30, mean = 0, sd = 5)
b = t.test(a, mu = 0, conf.level = 0.95)  
...
want the estaimte of u and the pvalue
